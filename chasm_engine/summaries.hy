(require chasm-engine.instructions [deftemplate def-fill-template])


(def-fill-template summary msgs-topic)
(def-fill-template summary msgs-points)
(def-fill-template summary msgs-paragraph)
(def-fill-template summary yes-no)
(def-fill-template summary complete-json)
(def-fill-template summary complete-lines)
(def-fill-template summary text-topic)
(def-fill-template summary text-points)
(def-fill-template summary text-extract)
(def-fill-template summary text)
